...
const API_BASE_URL = "https://urbanissuereporter-86jk0m0e.b4a.run/api";
...
const Map = () => {
    ...
    const fetchReclamations = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/all-reclamations-by-region/${userData.id}`
        );
        ...
        if (isMounted && data.reclamations) {
          ...
          const processedReclamations = data.reclamations
            .map((rec) => ({
              ...rec,
          ...
          const firstValidRec =
            processedReclamations.find(
              (rec) =>
                rec.parsedLocation &&
                (rec.statut === "en_attente" || rec.statut === "en_cours")
  ...
  const filteredReclamations = useMemo(
    () =>
      reclamations.filter((reclamation) => statusFilters[reclamation.statut]),
    [reclamations, statusFilters]
  );

  // Check if we have valid reclamations after filtering
  const hasNoReclamationsToShow =
            ...
            >
              <CardContent sx={{ p: 2, "&:last-child": { pb: 2 } }}>
                ...
                >
                  ...
                  >
                    <Box sx={{ color: data.color }}>{data.icon}</Box>
                  </Box>
                </Box>
              </CardContent>
            </Card>
      ...
      >
        ...
        {hasNoReclamationsToShow && (
          ...
          >
            ...
            <Typography
              ...
              textAlign="center"
            >
              Veuillez vérifier vos filtres ou ajouter des réclamations avec
              coordonnées valides.
            </Typography>
          </Box>
        )}
        ...
        <div
          style={{
            width: "100%",
            height: "100%",
            visibility: isMapLibraryLoaded ? "visible" : "hidden",
          }}
        >
          {isMapLibraryLoaded && (
            <GoogleMap
                  ...
                  >
                    ...
                    <Box
                      sx={{
                        ...
                        width: "100%",
                        backgroundColor:
                          STATUS_DATA[selectedReclamation.statut].color,
                      }}
                    />
                    ...
                    <Box sx={{ p: 2 }}>
                        ...
                        <Tooltip title="Localisation">
                          ...
                          >
                            ...
                            <Typography variant="caption" noWrap>
                              {selectedReclamation.parsedLocation.lat.toFixed(
                                6
                              )}
                              ,{" "}
                              {selectedReclamation.parsedLocation.lng.toFixed(
                                6
                              )}
                            </Typography>
                          </Box>
                      ...
                      <Button
                        ...
                        fullWidth
                        onClick={() =>
                          handleViewDetails(selectedReclamation.id)
                        }
                        sx={{
                          fontWeight: "bold",
                          boxShadow: 2,
