...
const Reclamation = () => {
    ...
    const fetchReclamation = async () => {
      try {
        const response = await axios.get(
          `https://urbanissuereporter-86jk0m0e.b4a.run/api/reclamation/${id}`
        );
        setReclamation(response.data.reclamation);

        // Récupération des photos liées à la réclamation
        const photosResponse = await axios.get(
          `https://urbanissuereporter-86jk0m0e.b4a.run/api/photos/reclamation/${id}`
        );
        ...
        setError(
          err.response?.data?.message || "Erreur lors de la récupération"
        );
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    const socket = io("https://urbanissuereporter-86jk0m0e.b4a.run"); // Assurez-vous que l'URL correspond à votre backend
  ...
  return (
    <Box sx={{ height: "100vh", overflow: "auto", padding: "20px" }}>
      <Header
        ...
        subtitle="Informations détaillées sur la réclamation"
      />
      ...
      <Paper
        ...
        sx={{
          ...
          borderRadius: "12px",
          backgroundColor:
            theme.palette.mode === "dark" ? colors.primary[500] : "#f5f5f7",
          mb: 4,
        }}
      >
                ...
                <Box sx={fieldStyle}>
                  ...
                  <Box
                    sx={{
                      ...
                      borderRadius: "20px",
                      bgcolor:
                        reclamation.reclamation.statut === "En cours"
                          ? colors.blueAccent[500]
                          : reclamation.reclamation.statut === "Résolu"
